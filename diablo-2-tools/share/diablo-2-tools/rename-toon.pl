...
open my $D2S_LOCATION_CONFIG_FILE, '</etc/diablo-2-tools/selected_char.conf' or die $!;
...
#if($ARGV[0] !~ /^[0-9a-zA-Z_-]{2,15}(\.d2s)?$/){
#  myError('first argument is a wrong name.');
#}

#if($ARGV[1] !~ /^[0-9a-zA-Z_-]{2,15}(\.d2s)?$/){
...
if(substr($ARGV[0], -4) eq '.d2s'){
  ...
  ($orig_d2s_character_name) = $orig_d2s_filename =~ /^([0-9a-zA-Z_-]+)/;
}
else{
...
if(substr($ARGV[1], -4) eq '.d2s'){
  ...
  ($new_d2s_character_name) = $new_d2s_filename =~ /^([0-9a-zA-Z_-]+)/;
}
else{
...
say('cd /usr/local/share/diablo-2-tools/');
