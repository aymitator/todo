...
open my $D2S_LOCATION_CONFIG, '</etc/diablo-2-tools/selected_char.conf' or die $!;
...
for my $i(0 .. $#JM_locations){
  ...
  if(length($sugar_slice_hex) == 28){
    ...
    system('perl', '/usr/local/share/diablo-2-tools/stam2rejuv.pl', '0x' . $sugar_slice_hex, $JM_locations[$i]);
    #exit(0);
  }
}
print "\n";
