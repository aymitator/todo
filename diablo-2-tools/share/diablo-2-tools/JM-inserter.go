...
func getManaCode(_g string) string {
  pre := "1000a0006500"
  post := "2807330302" 
       //  38b7a60702
  middle := ""
  ...
  if middle == "" {
    panic("invalid mana name (must be between 1 and 12)")
    return "" //what a strange gem!
  }
  return pre + middle + post
}
func getMoneyCode(_g string) string {
  pre := "1000a0006500"
  post := "38b7a60702"
  middle := ""
  if _g == "money1" {
  ...
  if middle == "" {
    panic("invalid money name (must be between 1 and 12)")
    return "" //what a strange gem!
  }
  return pre + middle + post
}
...
func main(){
  ...
  } else if strings.HasPrefix(FirstItem, "money") {
    FirstItem = getMoneyCode(FirstItem)
    p("Money code (ie. a Perfect Skull in the cube somewhere) is " + FirstItem)
  } else if strings.HasPrefix(FirstItem, "mana") {
    FirstItem = getManaCode(FirstItem)
    p("Mana code (ie. a Perfect Skull in the cube somewhere) is " + FirstItem)
  }
  ...
  //read the /etc/diablo-2-tools/selected_char.conf into the d2s_char_location variable
  // if we're on Windows read C:\cygwin64\etc\diablo-2-tools\selected_char.conf
  var d2s_file_location = ""
  if runtime.GOOS == "linux" {
    d2s_char_location_contents, err := ioutil.ReadFile("/etc/diablo-2-tools/selected_char.conf")
    check(err)
    d2s_file_location = string(d2s_char_location_contents)
  } else { //let's pray this is windows
    d2s_char_location_contents, err := ioutil.ReadFile("C:\\cygwin64\\etc\\diablo-2-tools\\selected_char.conf")
    check(err)
    d2s_file_location = string(d2s_char_location_contents)
  ...
  if len(d2s_file_location) == 0 {
    panic("Could not get d2s file location from /etc/diablo-2-tools/selected_char.conf")
  }
